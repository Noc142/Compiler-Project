...
void ExtDef(Node* node)
{
	...
	switch (node->npro) {
		...
		case 2:if (type == NULL) free(type); break;
...
void FunDec(Node* node, Type type,int isDefine)
{
	...
	{	
		...
		else { SymInsert_func(symtable, node->child->val, type, field, isDefine, node->child->lineno);}
...
Type Exp(Node* node)
{
	...
	case 14: {Type tp1 = Exp(node->child); Type tp2 = Exp(node->child->next->next); 
		...
		else return tp1->u.array.elem;//is this corrent??????????
	}
		break;
		//*****************************************************************************************************************************
	...
	default: {Type tp1 = Exp(node->child); Type tp2 = Exp(node->child->next->next);
		if (checkType(tp1, tp2) == 0||(checkType(tp1,type_int)==0&&checkType(tp1,type_float)==0))//only int or float can do arithmetic operation
			fprintf(stderr, "Error type 7 at Line %d: Type mismatched for operands.\n", node->child->lineno);
